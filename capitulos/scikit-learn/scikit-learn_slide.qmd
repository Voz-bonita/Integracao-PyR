...
-   **Classificação:** regr. logística, árvore de decisão, SVM, naive Bayes, LDA/QDA etc;
...
Caso o pacote scikit-learn não esteja instalado no Python, podemos usar o reticulate para fazer isso.
...
Para exemplificar o uso do sklearn, utilizaremos um data frame carregado dentro no R, que será passado para o ambiente Python como um data frame `pandas`.
...
-   <div>
    ...
    > Tanto `n_components` no PCA quanto o `C` no LinearSVC são hiperparâmetros desse modelo, e podem ser ajustados utilizando métodos de validação cruzada sobre o pipeline.

    </div>

## 

Para ajustar o modelo, utilizamos os dados de **treino**.
...
-   <div>
    ...
    > **(Vazamento de dados)**. Veja que o pipeline ajuda a reforçar o uso de apenas os dados de **treino** para o ajuste do modelo.
...
Podemos enfim trazer alguma parte de interesse do resultado para o R, por exemplo, as previsões do modelo.
...
ggplot(data=breast_cancer, aes(x=y_pred, y=y))+
  geom_jitter()
```

# Materiais

1.  Para mais informações sobre a biblioteca scikit-learn, veja sua [página](https://scikit-learn.org/stable/getting_started.html) na web e o [guia de usuário](https://scikit-learn.org/stable/user_guide.html);
